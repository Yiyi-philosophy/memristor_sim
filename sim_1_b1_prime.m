...
% scale 0.1 time 500 13140s = 3.7h
...
function main  
    ...
    %  变量范围  
    % b1_prime 2500
    sample_range = linspace(1000, 4000, sample_num);  
      
    % 存储SI结果  
    SI_results = zeros(length(sample_range), 1);  
      
    if parallel
        parfor k = 1:length(sample_range) 
            display(['proc:', num2str(k), '/', num2str(length(sample_range))])
            % V_D = sample_range(k);  
            b1_prime = sample_range(k);  
            % ODE求解  
    ...
    % 绘制SI图  
    figure;  
    plot(sample_range, SI_results, '-o');  
    xlabel('V_D');  
    ylabel('SI');  
